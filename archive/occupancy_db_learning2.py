...
# MOCK EVENT STREAM - Returns JSON strings (realistic API format)
def mock_occupancy_stream():
    """
    Generator that yields scan events as JSON strings.
...
def parse_scan_event(json_string: str) -> Dict:
    """
    Parse a JSON scan event string into a Python dictionary.

    In a real FastAPI application, this happens automatically:
    @app.post("/api/scans")
    async def create_scan(scan: ScanEvent):  # Pydantic model auto-parses JSON
        ...
    """
...
def populate_database():
    ...
    for event_json in mock_occupancy_stream():
        event = parse_scan_event(event_json)  # Parse JSON string to dict
        cursor.execute("""
            INSERT INTO scans (ticket_id, gate, scan_type, scan_time, user_id)
            VALUES (%s, %s, %s, %s, %s)
        """, (
            event['ticket_id'],
            event['gate'],
...
def get_occupancy_statistics_db():
    ...
    cursor.execute("SELECT COUNT(*) FROM scans WHERE scan_type = 'entry'")
    stats['total_entries'] = cursor.fetchone()[0]

    cursor.execute("SELECT COUNT(*) FROM scans WHERE scan_type = 'exit'")
    stats['total_exits'] = cursor.fetchone()[0]

    cursor.execute("""WITH current_occupancy AS
                   (SELECT DISTINCT ON(ticket_id) ticket_id, scan_type 
                   FROM scans ORDER BY ticket_id, scan_time DESC) 
                   SELECT COUNT (*) FROM current_occupancy WHERE scan_type = 'entry'""")
    stats['current_occupancy'] = cursor.fetchone()[0]
...
if __name__ == "__main__":
    ...
    print('find_people_still_inside_db() result: ', get_occupancy_statistics_db())
